...
function buildCutPointPrompt({ title, entity, situation, channel, sentences }) {
  ...
  return [
    ...
    "- Each scene spans one or more consecutive sentences (sentence_start <= sentence_end). Group sentences into scenes by beat / natural narration chunk. A single sentence stating one quantity (a duration, distance, proportion, count, or trend) SHOULD be its OWN one-sentence scene with render.subject_type \"data_visual\" (see the data_visual rules below).",
