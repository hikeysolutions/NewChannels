...
- **GPT Image 2 via Atlas Cloud (follow-up code task, not started).** Add a new adapter entry to `config/image-models.json` for GPT Image 2 via Atlas Cloud, and wire it as a real, tested option alongside `gemini-nb2-batch` and the existing OpenAI-direct stub. There are three distinct routes to the same underlying model; do not confuse them:
  ...
  Quality tier must be chosen per channel, not assumed uniform. Channel A's stick-figure style is hypothesized to hold at low quality (~$0.22 for a 48-shot video vs Gemini's current $0.816) but is untested. FailNation's photorealistic needs should be tested starting at medium, since low-quality photoreal is a real risk worth verifying before assuming it holds.
