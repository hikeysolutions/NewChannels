...
- **GPT Image 2 via Atlas Cloud (follow-up code task, not started).** Add a new adapter entry to `config/image-models.json` for GPT Image 2 via Atlas Cloud, and wire it as a real, tested option alongside `gemini-nb2-batch` and the existing OpenAI-direct stub. There are three distinct routes to the same underlying model; do not confuse them:
  ...
  Test results on the "low" tier (real two-prompt pairs through the actual Atlas Cloud Developer endpoint, not a wrapped tool; confirmed cost $0.00451/run):
  - Character/face/hair and clothing/props held excellently across dramatic pose, action, lighting, and emotional shifts, in BOTH test pairs, including photorealistic content. Character/wardrobe consistency, the thing that actually matters for audience recognition across a short, is reliable at low quality.
  - Background/environment fidelity is the one consistently weak point. Named, supposedly-fixed environment details (a backyard picnic table in the first test, a specific couch/location in the second) drifted between generations in BOTH tests even when prompts explicitly said "same setting" / "same room". This is a confirmed, repeatable limitation of low quality, not a hypothetical risk.
  Practical implication for FailNation (photorealistic): character/wardrobe consistency is reliable at low quality, so low is now a reasonably strong default for FailNation too, not just Channel A. But background/prop consistency should NOT be relied on for any gag or continuity detail that depends on a specific environment or object staying visually identical across shots. If a script needs a specific recurring prop or setting to look the same on screen, either accept the drift risk, generate that shot with maximal environment specificity, or check a higher quality tier before shipping that particular shot.
  Working assumption: "low" is a reasonably strong default for BOTH Channel A (stick-figure, low-risk regardless) and FailNation, pending 2 to 3 more test pairs across different characters/actions/settings before it is treated as fully locked. The third test pair (pool/fall scenario) is still pending.
