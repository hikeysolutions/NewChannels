...
| NEW — 00c, block-level schema | Added `gap_type` and `gap_state` fields to the emitted JSON manifest schema, replacing the independent-unit "curiosity loop" concept evaluated and rejected this session with an overlapping gap-chain model: each block may open a new tension before the prior one fully resolves. |
| NEW — 00c, `qa_flags` | Added a `category` column and three named flag categories: `factual_accuracy` (pre-existing, now named), `gap_logic`, `voice_consistency` — QA pass now checks narrative-chain integrity and persona adherence, not only factual claims. |
...
CREATE TABLE videos (
  ...
  approved_at TEXT,
  published_at TEXT,
  parent_video_id INTEGER REFERENCES videos(id),  -- v2.7: NULL for long-form; parent's id for a short
  video_type TEXT DEFAULT 'long_form'             -- v2.7: 'long_form' | 'short'
);
...
CREATE TABLE qa_flags (
  ...
  flagged_at TEXT DEFAULT (datetime('now')),
  category TEXT                        -- v2.7: 'factual_accuracy' | 'gap_logic' | 'voice_consistency'
);
```

### v2.7 schema additions — multi-shorts + gap-chain

Additive to the schema above. No existing data affected — `videos` has zero rows at merge time (see v2.6 → v2.7 changelog).

**`videos` — two new columns** (reflected in the CREATE TABLE above; applied to the live DB via):

```sql
ALTER TABLE videos ADD COLUMN parent_video_id INTEGER REFERENCES videos(id);
ALTER TABLE videos ADD COLUMN video_type TEXT DEFAULT 'long_form';  -- 'long_form' | 'short'
```

- Long-form: `parent_video_id = NULL`, `video_type = 'long_form'`.
- Each short cut from a long-form gets its own row: `parent_video_id` → the parent, `video_type = 'short'`, own cost tracking, own `status`, own `approved_at`/`published_at`.
- Cloudinary folder structure becomes `new-channels/[channel]/[parent_video_id]/` — the long-form render plus all shorts cut from it, grouped rather than flat.
- Telegram approval bundles all items from one production run into a single message (see Section 03a).

**Manifest JSON — two new per-block fields**, emitted by `flyt-script-generator.js`:
...
- `gap_state` — `opens | partial_resolve | resolves`. Defines the overlapping-gap chain: a block may `open` a new tension before the previous block's gap has fully `resolved`. Replaces the rejected fully-independent "any block, any order" model — Channel B's ascending Level Badge requires sequential order.
- Assembly layer (Remotion/HyperFrames, Section 09) requires **no change** — gap-chaining is a script/narration-layer capability only. The Level Badge continues to animate on block-transition beats exactly as specified in the prior revision.

**`qa_flags` — named categories.** The v2.7 design framed these as free-text usage of `flagged_claim` (no schema change), but this build adds an explicit `category` column (see the CREATE TABLE above) so the three categories can be filtered distinctly:

- `factual_accuracy` — unsupported/invented claims (pre-existing, Section 03 step 2).
- `gap_logic` — missing gap signal, repeated `gap_type` consecutively, no escalation pattern, stagnant `gap_state`.
- `voice_consistency` — persona drift, Forbidden Shortcut usage, failure against the Voice Consistency Score (Section 06).
...
5. `performance` data (Section 05b) feeds back against `entity_situation_bank.used_count` to show which combos are actually working — this is what the no-growth threshold decision (Section 06a) gets measured against, with real numbers instead of gut feel.
...
**Consistency rule:** same as Channel A — locked unless no-growth threshold triggers a full stop (see Section 06a).
...
### Multi-shorts bundling (v2.7)

Supersedes the single-asset-per-message assumption above. When a production run yields a long-form video plus one or more shorts cut from gap-boundary points (Section 00c, `gap_type`/`gap_state`):
...
5. All other approval-gate behavior above (unchanged) applies per item.
...
- Use this to inform the **no-growth threshold** decision (Section 06a) with real data rather than gut feel — e.g., which entity/situation combinations on Channel A are actually outperforming, which countdown topics on Channel B are underperforming.
...
**v2.7 additions (added, not replacing the list below):**

- [ ] **Build `ChannelB/STYLE_GUIDE.md` — currently does not exist.** Confirmed via Claude Code audit: `ChannelB/` contains only `REFERENCES.md` and `tasks/lessons.md`. No color theme, no thumbnail spec, no visual layout defined for Channel B beyond Section 01/07's high-level formula. Real production blocker distinct from naming — Channel B cannot render a real video without it regardless of what it's called.
- [ ] **Apply the `videos` and manifest schema additions (Section 00c)** — `parent_video_id`, `video_type`, `gap_type`, `gap_state`, and `qa_flags.category` — to `agents/lib/db.js` (`insertVideo`, `pickCombo`, `markComboUsed`) and `flyt-script-generator.js`'s output. Free to do now — zero rows in `videos`, zero migration risk.
- [ ] **Reconcile filename vs. title mismatch** — this file is saved as `New_Channels_Master_Build_v2_7.md`; the stale `New Channels Master Build v2 5.md` copy is archived.
- [ ] **Decide whether `entity_situation_bank` needs Channel B seed rows now or waits on the style guide.** Currently zero Channel B rows; 7 Channel A rows seeded and unused.
- [ ] **Channel B naming — deferred, not resolved.** "Ten Deep" rejected (ties the name to a fixed count). Direction agreed: a durable media-company/show label decoupled from any specific structural mechanic. Two externally-proposed names (**"Archive Zero"** for A, **"Critical Path"** for B) checked and **both unavailable** (Archive Zero collides with an existing AI-narrated documentary channel; Critical Path is saturated). Channel A's locked name ("Before Now") is unaffected — independently confirmed clear. Full naming exercise picked up as its own task, not bundled into pipeline/schema work.

**Original list (v2.6 and earlier):**
...
*Supersedes v2.6 for all sections listed in the v2.6 → v2.7 changelog above. All other sections carry forward unchanged. Cross-references ChannelB SPEC v2.0 for Channel B's remaining pipeline detail — note that SPEC v2.0 Section 06 (`VOICE_IDENTITY`) is now superseded by this document's Section 06 and should be treated as historical/removed on next SPEC revision.*
